<x-layout>
    ...
    <div class="p-6 lg:p-20">
        <h1 class="text-4xl text-pink-950 font-bold">{{__('services.careers')}}</h1>
        ...
        <div class="overflow-x-auto">
            ...
            @else
                <table class="table-auto w-full border-collapse border border-gray-300 mt-6 cursor-pointer">
                    <thead>
                    <tr>
                        <th class="text-left bg-gray-100 border-b border-gray-300 px-4 py-2">{{__('services.position')}}</th>
                        <th class="text-left bg-gray-100 border-b border-gray-300 px-4 py-2">{{__('services.description')}}</th>
                        <th class="text-left bg-gray-100 border-b border-gray-300 px-4 py-2">{{__('services.location')}}</th>
                        <th class="text-left bg-gray-100 border-b border-gray-300 px-4 py-2">{{__('services.created_at')}}</th>
                    </tr>
                    </thead>
                    <tbody>
        ...
        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center w-full space-x-5 mt-20">
            <div class="lg:w-1/3">
                <h1 class="uppercase tracking-wide text-3xl mt-10 text-center lg:text-start lg:mt-0">{{__('dashboard.contact')}}</h1>
                <p class="mt-10 text-center lg:text-start">{{__('services.send_cv')}}
                    <span class="font-bold">{{__('services.CV')}}</span></p>
            </div>
            <form  action="{{route('vacancy-response.store')}}" class="lg:w-1/2 bg-gray-50 shadow px-10 pb-10"
                ...
                <div class="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4 mt-10">
                    <div class="flex flex-col w-full lg:w-1/2">
                        <label for="name" class="text-sm text-pink-950 font-bold mb-1">{{__('dashboard.name')}} <span
                    ...
                    <div class="flex flex-col w-full lg:w-1/2">
                        <label for="email" class="text-sm text-pink-950 font-bold mb-1">{{__('dashboard.email')}} <span
                                class="text-red-700">*</span></label>
                ...
                <div class="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4 mt-10">
                    <div class="flex flex-col w-full lg:w-1/2">
                        <label for="position" class="text-sm text-pink-950 font-bold mb-1">{{__('services.position')}} <span
                                class="text-red-700">*</span></label>
                        <input type="text" name="position" id="position" class="border border-gray-300 p-3 rounded-md"
                               placeholder="{{__('services.enter_position')}}">

                    </div>
                    <div class="flex flex-col w-full lg:w-1/2">
                        <label for="CV" class="text-sm text-pink-950 font-bold mb-1">{{__('services.upload_cv')}} <span
                                class="text-red-700">*</span></label>
